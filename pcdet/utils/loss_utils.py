    ...
    def __init__(self, reduction = 'none'):
        ...
        Args:
            code_weights: (#codes) float list if not None.
                Code-wise weights.
        """
        super(L1LossCustom, self).__init__()
        assert reduction in ['none', 'sum', 'mean', 'torch_mean']
        self.reduction = reduction

    @torch.cuda.amp.custom_fwd(cast_inputs=torch.float16)
    def forward(self, input: torch.Tensor, target: torch.Tensor, mask : torch.Tensor, code_weights: torch.Tensor, ind : torch.Tensor = None):
        """
        Args:
            input: (B, #anchors, #codes) float tensor.
                Ecoded predicted locations of objects.
        ...
        loss = loss * bbox_weights

        if self.reduction == 'mean':
            num = mask.float().sum()
            eps = torch.finfo(torch.float32).eps
            loss = loss.sum() / (num + 1e-4 + eps)
        elif self.reduction == 'sum':
            loss = loss.sum()
        elif self.reduction == 'torch_mean':
            loss = loss.mean()
