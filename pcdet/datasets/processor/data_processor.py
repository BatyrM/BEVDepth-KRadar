...
class DataProcessor(object):
    ...
    def get_depth(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.get_depth, config=config)

        img_aug_matrix = torch.from_numpy(np.stack(data_dict['img_aug_matrix'], axis=0)).type(torch.float32)
        bev_aug_matrix = torch.from_numpy(data_dict['lidar_aug_matrix']).type(torch.float32)
        lidar2image = torch.from_numpy(np.stack(data_dict['lidar2image'], axis=0)).type(torch.float32)

        points = torch.from_numpy(data_dict['points']).type(torch.float32)
        img = torch.from_numpy(np.stack(data_dict['camera_imgs'], axis=0)).type(torch.float32)

        if config.get('KEYFRAME_ONLY', False):
        ...
        # print("CC", cur_coords.dtype)
        # print("IAM", img_aug_matrix.dtype)
        # print("BAM", bev_aug_matrix.dtype)
        # print("L2I", lidar2image.dtype)
        # print("P", points.dtype)
