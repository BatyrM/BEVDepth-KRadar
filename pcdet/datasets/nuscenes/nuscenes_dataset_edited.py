...
class NuScenesDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, return_depth = False):
    ...
    def load_camera_info(self, input_dict, info, points):
        ...
        for _, camera_info in info["cams"].items():
            ...
            if self.return_depth:
                point_depth = self.get_lidar_depth(points, Image.fromarray(np.array(Image.open(camera_info["data_path"]))), info['lidar_infos'], camera_info)
        ...
        if self.return_depth:
            input_dict["lidar_depth"] = []
            for i, (_, camera_info) in enumerate(info["cams"].items()):
                point_depth = self.get_lidar_depth(points, input_dict['camera_imgs'][i], info['lidar_infos'], camera_info)
                point_depth_augmented = depth_transform(point_depth, resize, _, crop, flip, rotate)
                input_dict["lidar_depth"].append(point_depth_augmented)
    ...
    def __getitem__(self, index):
        ...
        data_dict['dataset_name'] = 'nuscenes'
