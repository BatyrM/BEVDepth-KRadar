...
class CenterHead(nn.Module):
    ...
    def build_losses(self):
        if self.model_cfg.LOSS_CONFIG.get('LOSS_CLS', None) is None:
            self.add_module('hm_loss_func', loss_utils.FocalLossCenterNet())
        else:
            hm_loss_func = getattr(loss_utils, self.model_cfg.LOSS_CONFIG.LOSS_CLS['loss_type'])
            self.add_module('hm_loss_func', hm_loss_func(reduction=self.model_cfg.LOSS_CONFIG.LOSS_CLS['reduction']))
        if self.model_cfg.LOSS_CONFIG.get('LOSS_REG', None) is None:
            self.add_module('reg_loss_func', loss_utils.RegLossCenterNet())
        else:
            reg_loss_func = getattr(loss_utils, self.model_cfg.LOSS_CONFIG.LOSS_REG['loss_type'])
            self.add_module('reg_loss_func', reg_loss_func(reduction=self.model_cfg.LOSS_CONFIG.LOSS_CLS['reduction']))
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            elif self.model_cfg.LOSS_CONFIG.LOSS_REG['loss_type'] == 'L1LossCustom':
                reg_loss = self.reg_loss_func(
                    pred_boxes, target_boxes, target_dicts['masks'][idx], 
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            for k, final_dict in enumerate(final_pred_dicts):
                ...
                if post_process_cfg.NMS_CONFIG.NMS_TYPE not in  ['circle_nms', 'class_specific_nms']:
                    ...
                    boxes = torch.cat([final_dict['pred_boxes'][:, [0, 1]], final_dict['pred_scores'].view(-1, 1)], dim = 1)
                    selected, selected_scores = model_nms_utils.circle_nms(
                        boxes=boxes,
                        scores=final_dict['pred_scores'],
                        min_radius=post_process_cfg.NMS_CONFIG.MIN_RADIUS[idx] if isinstance(post_process_cfg.NMS_CONFIG.MIN_RADIUS, list) else post_process_cfg.NMS_CONFIG.MIN_RADIUS,
