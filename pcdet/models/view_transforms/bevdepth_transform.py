...
class BevDepthTransform(nn.Module):
    """
    ...
    def __init__(self, model_cfg):
        ...
        self.use_depth = self.model_cfg.get('USE_DEPTH', False)
        ...
        # print( "Shape of Frustume is : " + str( self.frustum.shape ))
    ...
    def get_cam_feats(self, x, d):
        ...
        # depth_tensor = torch.mean(x[0][0], dim = 3)
        # print("Over C:", depth_tensor.shape)
        # import numpy as np
        # import matplotlib.pyplot as plt
        # depth_map_c = x[0][0][:,:,:,0].detach().cpu().numpy()
        # depth_map_c = torch.mean(x[0][0], dim = 3).detach().cpu().numpy()
        # depth_map_2d = np.mean(depth_map_c, axis=0)
        # depth_map_2d_norm = (depth_map_2d - depth_map_2d.min()) / (depth_map_2d.max() - depth_map_2d.min())
        # plt.imshow(depth_map_2d_norm, cmap='plasma')  # You can change the colormap if needed
        # plt.colorbar()
        # plt.title("2D Depth Map Visualization")
        # plt.show()
    ...
    def forward(self, batch_dict):
        ...
        # print(batch_dict['camera_intrinsics'])
        # print(batch_dict['camera2lidar'])
        # print(batch_dict['lidar2image'])
        ...
        #print("CAMERA 2 LIDAR SHAPE:", camera2lidar.shape)
        ...
        # batch_size = BN // 6
        #batch_size = BN // 6 if BN >= 6 else BN
        batch_size = x.size(0)
        ...
        for b in range(batch_size):
            ...
            # print("CUR COORDS 1:", cur_coords.shape)
            # print("CUR IMG AUG MAT:", cur_img_aug_matrix.shape)
            # print("CUR LIDAR AUG MAT:", cur_lidar_aug_matrix.shape)
            # print("CUR LIDAR 2 IMAGE:", cur_lidar2image.shape)
            ...
            # print("CUR COORDS 2:", cur_coords.shape)

            # lidar2image
            cur_coords = cur_lidar2image[:, :3, :3].matmul(cur_coords)
            cur_coords += cur_lidar2image[:, :3, 3].reshape(-1, 3, 1)
            
            # print("CUR COORDS 3:", cur_coords.shape)
            ...
            # print("CUR COORDS 4:", cur_coords.shape)

            # do image aug
            cur_coords = cur_img_aug_matrix[:, :3, :3].matmul(cur_coords)
            cur_coords += cur_img_aug_matrix[:, :3, 3].reshape(-1, 3, 1)

            # print("CUR COORDS 5:", cur_coords.shape)

            cur_coords = cur_coords[:, :2, :].transpose(1, 2)

            # print("CUR COORDS 6:", cur_coords.shape)

            # normalize coords for grid sample
            cur_coords = cur_coords[..., [1, 0]]

            # print("CUR COORDS 7:", cur_coords.shape)
            ...
            # print("ON IMG:", on_img.shape)
            for c in range(on_img.shape[0]):
                masked_coords = cur_coords[c, on_img[c]].long()
                masked_dist = dist[c, on_img[c]]
                # print("SHAPES:", depth.shape, masked_coords.shape, masked_dist.shape)
