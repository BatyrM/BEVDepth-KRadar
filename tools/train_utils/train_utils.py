...
def train_one_epoch2(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    rank, tbar, total_it_each_epoch, dataloader_iter, tb_log=None, leave_pbar=False, 
                    use_logger_to_record=False, logger=None, logger_iter_interval=49, cur_epoch=None, 
                    total_epochs=None, ckpt_save_dir=None, ckpt_save_time_interval=300, show_gpu_stat=False, 
                    use_amp=False, grad_acc=False, acc_factor = None):

    assert grad_acc and acc_factor is not None
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if (cur_it + 1) % acc_factor == 0 or (cur_it + 1) == total_it_each_epoch:
            ...
            if rank == 0:
                ...
                forward_time.update(avg_forward_time)
                batch_time.update(avg_batch_time)
                losses_m.update(loss.item() , batch_size * acc_factor)
                if 'loss_heatmap' in list(tb_dict.keys()) and 'loss_cls' in list(tb_dict.keys()):
                    heatmap_loss_disp.update(tb_dict['loss_heatmap'])
                    cls_loss_disp.update(tb_dict['loss_cls'])
                    disp_dict.update({
                ...
                if use_logger_to_record:
                    # if accumulated_iter % logger_iter_interval == 0 or cur_it == start_it or cur_it + 1 == total_it_each_epoch:
                    if (cur_it + 1) % logger_iter_interval or cur_it + 1 == total_it_each_epoch:
                        trained_time_past_all = tbar.format_dict['elapsed']
                        second_each_iter = pbar.format_dict['elapsed'] / max(cur_it - start_it + 1, 1.0)
...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                ...
                grad_acc=False, acc_factor=None):
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            if grad_acc and acc_factor is not None:
                accumulated_iter = train_one_epoch2(model, optimizer, train_loader, model_func,
